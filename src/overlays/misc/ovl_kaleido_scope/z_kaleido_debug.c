...
static u16 sRowFirstSections[] = {
    SECTION_RUPEES,
    SECTION_FIRST_INVENTORY_SLOT,
    SECTION_FIRST_KEY_COUNT,
    SECTION_FIRST_UPGRADE,
    SECTION_FIRST_EQUIPMENT,
    SECTION_FIRST_DUNGEON_ITEMS,
    SECTION_FIRST_MEDALLION,
    SECTION_FIRST_SONG,
    SECTION_FIRST_SPIRITUAL_STONE,
    SECTION_STONE_OF_AGONY,
    SECTION_HEART_PIECES,
};
    ...
    switch (curSection) {
        ...
        default:
            if (curSection <= SECTION_LAST_INVENTORY_SLOT) {
                ...
                } else if (i < SECTION_FIRST_KEY_COUNT) { // should probably be curSection < SECTION_FIRST_KEY_COUNT
                    if (CHECK_BTN_ALL(input->press.button, BTN_CUP) || CHECK_BTN_ALL(input->press.button, BTN_CLEFT) ||
                ...
                if (curSection <= SECTION_LAST_EQUIPMENT) {
                    ...
                    if ((curSection >= SECTION_FIRST_UPGRADE) && (curSection <= SECTION_LAST_UPGRADE)) {
                        ...
                        i = curSection - SECTION_LAST_UPGRADE; // 0 <= i < 4
                        if (CHECK_BTN_ALL(input->press.button, BTN_CLEFT)) {
